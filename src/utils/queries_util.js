...
const configured_anime_scraping =  `SELECT as2.season, a.title, as2.chapter_number,as2.chapter_name, as2.chapter_image,as2.chapter_link, 
                                    CASE as2.clicked 
                                    WHEN '1' THEN true 
...
`SELECT usd.id AS id, usd.value AS value, usd.name AS name, DATE_FORMAT(usd.date , "%Y-%m-%d") AS date 
    FROM 
